...
class FavoriteViewModelTest {
    ...
    @ExperimentalCoroutinesApi
    @Test
    fun insertToFavorite_insertItem_returnTrue() =  runTest{
...
// WHEN - Get the task  from the database and collect it
        viewModel.getFromFFavorite()

        when(viewModel.data.value ){
            is ApiStateFav.Success->{
                t= (viewModel.data.value as ApiStateFav.Success).dataState.size
            }
            is ApiStateFav.Loading ->{
    ...
    @Test
    fun deleteFromFav_deleteItem_returnTrue() = runTest {
        //GIVEN init data , view model , fack repo , insert data
        repository= FakeRepository(favList, alarmList,roomResponse,weatherResponse)
        viewModel= FavoriteViewModel(repository)
